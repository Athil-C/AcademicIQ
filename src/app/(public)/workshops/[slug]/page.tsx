...
import { Layers, Calendar, Clock, User, ExternalLink, ArrowLeft } from "lucide-react";
...
export default async function WorkshopDetailPage({
  ...
  const spec = item.workshop;

  return (
    <div className="py-10 bg-slate-50 min-h-screen">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="flex items-center gap-2 text-xs text-slate-500">
          <Link href="/workshops" className="hover:text-blue-700 flex items-center gap-1 font-medium">
        ...
        <article className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 sm:p-10 space-y-8">
          <div className="space-y-3 border-b border-slate-100 pb-6">
            <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-800 border border-amber-100 inline-flex items-center gap-1">
            ...
            {spec?.speaker && (
              <div className="flex items-center gap-1.5 text-xs text-slate-600 font-medium">
                ...
                <span>Instructor / Lead: {spec.speaker}</span>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-xs bg-slate-50 p-4 rounded-xl border border-slate-200">
            ...
            <div>
              ...
              <span className="font-semibold text-slate-800">{spec?.mode || "Online"}</span>
            </div>
          </div>

          <div className="space-y-4">
            <h2 className="text-lg font-bold font-serif text-slate-900">Syllabus & Prerequisites</h2>
            {item.content ? (
